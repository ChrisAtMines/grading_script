...
let exit_status = if !Flags.test && List.length !Flags.mode > 0 then (
  ...
  | (Lab03) ->
    reset_test_counts ();
    let _ = list_check Lab03.simple_to_num_tests in
    let _ = list_check Lab03.simple_to_bool_tests in
    let _ = list_check Lab03.simple_to_str_tests in
    let _ = list_check Lab03.simple_expr_eval_tests in
    let _ = list_check Lab03.simple_print_eval_tests in
    let _ = list_check Lab03.simple_cond_eval_tests in
    let _ = list_check Lab03.simple_str_eval_tests in
    let _ = list_check Lab03.simple_var_eval_tests in
  ...
  List.iter (fun mode ->
    ...
    | (Lab03) ->
      ...
      let v = Lab03.eval (empty_env,input_ast) in
