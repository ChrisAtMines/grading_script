...
let exit_status = if !Flags.test && List.length !Flags.mode > 0 then (
  ...
  | (Lab02) ->
    reset_test_counts ();
    let _ = list_check Lab02.simple_eval_tests in
    let _ = list_check Lab02.simple_typecheck_tests in
  ...
  List.iter (fun mode ->
    ...
    | Lab02 ->
      ...
      let e = (get_expr input_ast) in
      let eval = Javascript_ast.str_value (Lab02.eval e) in
      let typ = str_option Javascript_ast.str_typ (Lab02.typecheck e) in
