...
let exit_status = if !Flags.test && List.length !Flags.mode > 0 then (
  ...
  List.iter (fun mode ->
    ...
    | (Lab04) ->
      ...
      let v = Lab04.eval (empty_env,input_ast) in
      let v_num = to_num v in
      let v_bool = to_bool v in
      let v_str = to_str v in
      let eval = Javascript_ast.str_value v in
      Printf.sprintf "------------------------------\nEVAL:    %s\n------------------------------\nto_num:  %s\nto_bool: %s\nto_str:  %s\n"(*"\nfinal env:\n%s\n"*)
        eval (str_float v_num) (string_of_bool v_bool) v_str (*(str_option str_environment !Lab04.final_env)*)
    ) in
    Printf.printf "%s\n" result
  ) !Flags.mode;
  0 (* exit status *)
